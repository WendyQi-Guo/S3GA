...
def get_arguments():
    ...
    # parser.add_argument("--save_folder", type=str, default='tmp_noseed_labse/')
    parser.add_argument("--save_folder", type=str, default='tmp_noseed_labse_smallclusternumber/')
    parser.add_argument("--result_folder", type=str, default='result_noseed_labse/')
...
from framework_noseed_labse import *
import time
import utils
...
def step2_batch_similarity():
    if skip_if_complete:
        ...
        if ok:
            ...
            return
    ea, embeddings = load_curr_objs(PHASE_TRAINING) # embeddings[1275304, 647946] 
    # # 重新保存一下embedding
    # saveobj(embeddings, save_folder + 'embeddings_medium_fr.pkl')
    # saveobj((ea.__dict__), save_folder + 'ea_medium_fr.pkl')
    # exit()
    # # load LaBSE embeddings
    # emb1_path = osp.join(f'/cmach-data/guowenqi/raw/{lang}_ent_labse_emb_1')
    # emb2_path = osp.join(f'/cmach-data/guowenqi/raw/{lang}_ent_labse_emb_2')
    ...
    #         x2 = pickle.load(f)
    # embeddings = (x1, x2)
    # assert embeddings[0].shape[0] == len(ea.ent1)
    # assert embeddings[1].shape[0] == len(ea.ent2)
    # ea, embeddings = load_curr_objs(PHASE_TRAINING)
    ...
    step2_time = time.time() - since

    add_log('step2_time', step2_time)
...
def step3_fuse_and_csls():
    ...
    final_csls, acc = framework.eval_sim(kmeans_xgb_partition_matrix)
