...
from metis_noseed_labse import Partition

from common.sinkhorn import *
from partition_models.trainer_noseed_labse import PartitionTrainer
...
class LargeFramework:
    def __init__(self, ea: EAData, max_sinkhorn_sz, device='cuda'):
        ...
        self.device = device
        self.ea = ea
        partition = Partition(ea, seed_rate=0.0)
        # self.tset, self.tmap = partition.test_set, partition.train_map #  size [442687, 442687], datasets 中的all pairs
        self.tset, self.lmap = partition.test_set, partition.link_map # without outliers now
    ...
    def get_partition_nodes(self, k=10, src=0,
                            embeddings=None,
                            partition_method='kmeans', model='xgb', 
                            model_dim=600, max_iter=500,
    ...
    def get_partition_similarity_matrix(self, k=10, src=0, top_k_corr=1,
        ...
        partition, src_nodes, trg_nodes, src_train, trg_train = \
            self.get_partition_nodes(k=k, src=src,
                                     ...
                                     max_iter=max_iter,
                                     bid=False)
        # def cluster_acc(src_nodes, trg_nodes, tmap):
        #     """src_nodes [[], [],...]
        #     """
        #     cluster_s = torch.full((sum([len(src_nodes[i]) for i in range(30)]), ), -1)
        #     for i in range(30):
        #         cluster_s[src_nodes[i]] = i  
        #     cluster_t = torch.full((sum([len(trg_nodes[i]) for i in range(30)]), ), -1)
        #     for i in range(30):
        #         cluster_t[trg_nodes[i]] = i    

        #     test_src_cluster = cluster_s[[key for key in tmap[0].keys()]]
        #     test_tgt_cluster = cluster_t[[key for key in tmap[0].values()]]
        #     acc = torch.sum(test_src_cluster == test_tgt_cluster) / torch.tensor(len(tmap[0]))
        #     return acc
        # cluster_acc = cluster_acc(src_nodes, trg_nodes, self.lmap)
        def cluster_acc(src_nodes, trg_nodes, link):
            """ src_nodes [[], [],...],
                link [(,), (,),...]
            """
            num = 0
            gt_y = torch.tensor(link).T
            for i in range(len(src_nodes)):
                src_idx = torch.isin(gt_y[0], torch.tensor(src_nodes[i]))
                num += torch.isin(gt_y[1][src_idx], torch.tensor(trg_nodes[i])).sum()
            return num / gt_y.shape[1]
            
        cluster_acc = cluster_acc(src_nodes, trg_nodes, self.ea.link)
        print(f'cluster_acc:', cluster_acc)
        add_log(f'cluster_acc_of_{partition_method}_cluster_acc', cluster_acc)
        tic = time.time()
    ...
    def get_eval_ids(self, ids1, ids2, src=0, keep_train=False):
        ...
        while curr < len(ids1):
            id1 = ids1[curr]
            if (id1 in tset[src] or keep_train) and lmap[src][id1] in ids2_set:
                new_ids1.append(id1)
            curr += 1
        curr = 0
        while curr < len(ids2):
            id2 = ids2[curr]
            if (id2 in tset[1 - src] or keep_train) and lmap[1 - src][id2] in ids1_set:
                new_ids2.append(id2)
            curr += 1
    ...
    def create_batch_sim(self, left, right, enhance='none', size=None, assoc0=None, assoc1=None, norm=False,
        ...
        if whole_batch:
            ...
            assoc0, assoc1 = self.get_eval_ids(assoc0, assoc1) # 取了原图目标图都有的点.
            if not isinstance(assoc0,torch.Tensor):
                assoc0 = torch.tensor(assoc0)
            if not isinstance(assoc1, torch.Tensor):
    ...
    #     corr_val, corr_ind = map(lambda x: x.numpy(), corr.topk(top_k_corr)) # value, index
    ...
    def get_partition_sim(self, left_embedding, right_embedding, partition,
        ...
        for i in range(len(src_nodes)):
            ids1 = src_nodes[i]
            ids2 = trg_nodes[i]
            # 这里用了label 信息，只保留了每个类中可以正确匹配的节点, 所以去掉
            # ids1, ids2 = self.get_eval_ids(ids1, ids2, keep_train=keep_train) # 去掉 outlier
            assoc0 = torch.tensor(ids1)
            assoc1 = torch.tensor(ids2)
            if len(ids1) == 0 or len(ids2) == 0:
                continue
